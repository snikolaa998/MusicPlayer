...
class MusicService : MediaBrowserServiceCompat() {
    ...
    override fun onCreate() {
        ...
        //get activity intent for notification -> click on notification open main activity
        ...
        musicNotificationManager = MusicNotificationManager(this, mediaSession.sessionToken, MusicPlayerNotificationListener(this)) {
            //Ovo je neka lambda funkcija koja se uvek poziva kada se trenutna pesma promeni(nije mi bas najjasnije)
            curSongDuration = exoPlayer.duration
        }

        val musicPlaybackPreparer = MusicPlaybackPreparer(firebaseMusicSource) {
            //Poziva se uvek kada player odabere novu pesmu
            curPlayingSong = it
     ...
     private inner class MusicQueueNavigator : TimelineQueueNavigator(mediaSession) {
         //Poziva se kada su nam neophodne nove informacije
    ...
    override fun onLoadChildren(parentId: String, result: Result<MutableList<MediaBrowserCompat.MediaItem>>) {
        when(parentId) {
            MEDIA_ROOT_ID -> {
                ...
                if (!resultSent) {
                    //Proverice kasnije da li je dostupan sadrzaj
                    result.detach()
                }
            }
        }
